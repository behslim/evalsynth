...
feedforward_network <- function(params) {
  ...
  if (params$dropout_active) {
    modules <- modules %>% append(nn_dropout(p=params$dropout_prob))
  }

  # Input layer

  modules <- modules %>% append(nn_linear(in_features=params$input_dim, out_features=params$num_hidden, bias=FALSE))
  modules <- modules %>% append(ACTIVATION_DICT[[params$activation]]())
  ...
  for (u in 1:(params$num_layers-1)) {
    ...
    modules <- modules %>% append(nn_linear(in_features=params$num_hidden, out_features=params$num_hidden, bias=FALSE))
    modules <- modules %>% append(ACTIVATION_DICT[[params$activation]]())
  }

  # Output layer

  modules <- modules %>% append(nn_linear(in_features=params$num_hidden, out_features=params$rep_dim, bias=FALSE))

  architecture <- nn_sequential()
...
OneClassLoss <- function(outputs, c) {
  dist   <- torch_sum((outputs - c) ** 2, dim=2)
  loss   <- torch_mean(dist)
...
SoftBoundaryLoss <- function(outputs, R, c, nu) {
  dist   <- torch_sum((outputs - c) ** 2, dim=2)
  scores <- dist - R ** 2
  loss   <- R ** 2 + (1 / nu) * torch_mean(torch_maximum(torch_zeros_like(scores), scores))

  scores <- dist
  loss   <- (1 / nu) * torch_mean(torch_maximum(torch_zeros_like(scores), scores))
...
BaseNet <- R6::R6Class(classname = "BaseNet",
                   ...
                   public = list(
                     initialize = function() {
                       ...
                       self$logger <- getLogger(name = deparse(self$classname))
...
OneClassLayer <- R6::R6Class(classname = "OneClassLayer",
                         ...
                         public = list(
                           ...
                           fit = function(x_train, verbosity = TRUE) {
                             ...
                             self$X <- torch::torch_tensor(x_train %>% matrix(ncol=self$input_dim), dtype = torch_float32())
                             ...
                             if (self$train_prop != 1) {
                               ...
                               inputs_val <- torch_tensor(x_val, requires_grad = TRUE)$to(dtype = torch_float32(),
                                                                                          device = self$device)
                             }
                             ...
                             for (epoch in 1:self$epochs){
                               ...
                               inputs <- torch_tensor(x_train, requires_grad = TRUE)$to(dtype = torch_float32(),
                                                                                        device = self$device)
                               ...
                               if (epoch >= self$warm_up_epochs && self$loss_type == "SoftBoundary") {
                                 dist <- torch_sum((outputs - self$c)^2, dim = 1)
...
compute_alpha_precision <- function(real_data, synthetic_data, emb_center, compute_authen = TRUE) {
  ...
  emb_center <- torch_tensor(emb_center, device=device)

  n_steps <- 30
  nn_size <- 2
  alphas <- seq(0, 1, length.out = n_steps)

  Radii <- as.matrix(torch_tensor(quantile(as.matrix(torch_sqrt(torch_sum((torch_tensor(real_data, dtype = torch_float32()) - emb_center)^2, dim = -1))), alphas)))

  alpha_precision_curve <- numeric()

  synth_to_center <- torch_sqrt(torch_sum((torch_tensor(synthetic_data, dtype = torch_float32()) - emb_center)^2, dim = -1))
  ...
  if (compute_authen == TRUE){
    ...
    real_to_real <- torch_tensor(real_to_real[,2])
    real_to_synth <- torch_tensor(real_to_synth)
    real_to_synth_args <- torch_tensor(real_to_synth_args)
...
compute_metrics <- function(real_data, synthetic_data, seed = 42, rep_dim = NULL, compute_authen = FALSE) {
  ...
  hyperparams$center <- torch_ones(params$rep_dim)
  ...
  X_out_real <- model_real$forward(torch::torch_tensor(X, dtype = torch_float32())) %>% torch_tensor(dtype = torch_float32()) %>% with_no_grad() %>% as.matrix()
  Y_out_real <- model_real$forward(torch::torch_tensor(Y, dtype = torch_float32())) %>% torch_tensor(dtype = torch_float32()) %>% with_no_grad() %>% as.matrix()
  ...
  X_out_synth <- model_synth$forward(torch::torch_tensor(X, dtype = torch_float32())) %>% torch_tensor(dtype = torch_float32()) %>% with_no_grad() %>% as.matrix()
  Y_out_synth <- model_synth$forward(torch::torch_tensor(Y, dtype = torch_float32())) %>% torch_tensor(dtype = torch_float32()) %>% with_no_grad() %>% as.matrix()
    ...
    if (!is.null(resamp.method) && resamp.method == "none") S_pMSE <- standard_pMSE <- NA
      ...
      for (i in 1:nperms) {
        ...
        if (method == "cart") {
          ...
          simpMSE[i] <- (sum((score - cc)^2, na.rm = T)) / N
          ...
          if (aggregate == TRUE) {
            simpMSE[i] <- sum(wt*(score - cc)^2, na.rm = T) / N  # reduced by factor of 2
          } else {
            simpMSE[i] <- sum((score - cc)^2, na.rm = T) / N  # reduced by factor of 2
          }
        }
      }
...
CISynthpop = function(original_data, synthetic_data, target, Measure, m){
  ...
  original_CI = confint(profile(results), level = 0.95)
  ...
  for (i in 1:m){
    ...
    synthetic_CI = confint(profile(results), level = 0.95)
...
Disclosure = function(orig, syn, sensitive_var, sensitive_conti_k){
  ...
  KMEANS = function(orig, syn, sensitive_var, sensitive_conti_k){
    ...
    clu_d <- clu_d %>% group_by(clu) %>% summarize(value = max(value)) %>% arrange(value)
...
pop_uni <- function(original_data, synthetic_data, conti_var=c(), K_list=FALSE){
  ...
  if (length(conti_var) == 0) {
    ...
    for (i in seq_along(conti_var)) {
      ...
      clu_d <- clu_d %>% group_by(clu) %>% summarise(value=max(value)) %>% arrange(value) %>% as.data.frame()
      clu_value <- clu_d$value
...
record_linkage <- function(x, x_link, block, scale){
  ...
  candidate_links <- foreach(i = which(Lmin>0), .combine = rbind) %dopar% {
  ...
  link_sc = link_D
  link_sc <- foreach(i = 1:length(comp_column), .combine = cbind) %dopar% {
...
compute_mia <- function(original_data, synthetic_data, threshold = 2) {
  ...
  bw <- sqrt(diag(Hns(x = as.matrix(original_data))))
  bw[bw == 0] <- 0.1

  orig_dens_est <- kde(x = as.matrix(original_data), H=diag(bw), eval.points = as.matrix(original_data), binned = FALSE)
  p_R <- orig_dens_est$estimate
  syn_dens_est <- kde(x = as.matrix(synthetic_data), H=diag(bw), eval.points = as.matrix(original_data), binned = FALSE)
...
##################################################
# Example , 최소한으로 필요한 parameter만 기입
##################################################
# load('DAT/est_survey2_sampled.RData')
# orig <- head(est_survey2_sampled, 1000)
# write.csv(orig, 'DAT/orig_sample.csv', row.names=FALSE)
# syn <- syn(orig)$syn
# write.csv(syn, 'DAT/syn_sample.csv', row.names=FALSE)

# orig_fact_df <- read.csv('DAT/orig_sample.csv', colClasses=c("factor", rep("numeric",18), "factor", "numeric"))
# orig_num_df <- data.frame(sapply(read.csv('DAT/orig_sample.csv'), as.numeric))
# syn_fact_df <- read.csv('DAT/syn_sample.csv', colClasses=c("factor", rep("numeric",18), "factor", "numeric"))
# syn_num_df <- data.frame(sapply(read.csv('DAT/syn_sample.csv'), as.numeric))
...
# print(paste('beta_recall:', result))
# result <- Compute_Measure(orig_num_df, syn_num_df, measure = 'cl_overlap', cl_target='BIS_MNTH')
# print(paste('cl_overlap:', result))
# # >> glm의 coefficient가 NA를 뱉는 경우 NA를 반환하게 작성되어 있음.
#
# # Risk
#
# result <- Compute_Measure(orig_fact_df, syn_fact_df, measure = 'idr', sensitive_var = 'BIS_MNTH')
# print(paste('idr:', result))
# result_list <- Compute_Measure(orig_fact_df, syn_fact_df, measure = 'cap', sensitive_var = 'BIS_MNTH')
# print(paste('cap:', result_list))
# # >> CAP_0, CAP_NA로 이루어진 리스트 반환
# result <- Compute_Measure(orig_fact_df, syn_fact_df, measure = 'pop_uni')
# print(paste('pop_uni:', result))
# result <- Compute_Measure(orig_num_df, syn_num_df, measure = 'record_linkage', sensitive_var = 'BIS_MNTH')
# print(paste('record_linkage:', result))
...
# print(paste('dcr:', result))
# result <- Compute_Measure(orig_num_df, syn_num_df, measure = 'nndr')$NNDR_mean
# print(paste('nndr:', result))
# # -->> DCR, NNDR이 list형태로 반환되며 각각 nrow의 길이를 갖고 있어 각각의 평균 및 전체값을 리스트 형태로 반환
# result <- Compute_Measure(orig_num_df, syn_num_df, measure = 'authen')
# print(paste('authen:', result))
